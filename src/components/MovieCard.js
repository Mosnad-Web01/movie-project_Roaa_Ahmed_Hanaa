...
const MovieCard = ({ item, isLoggedIn, media_type }) => {
  ...
  // تحديد الحقل الصحيح لتاريخ الإصدار بناءً على نوع الوسائط
  const releaseDate = item.release_date || item.first_air_date 
    ? new Date(item.release_date || item.first_air_date).toDateString() 
    ...
    <div className="relative bg-white dark:bg-gray-800 text-black dark:text-white rounded-lg shadow-md overflow-hidden w-[300px] h-[450px]">
      ...
      <div className="absolute top-2 right-2">
        ...
        {dropdownVisible === item.id && (
          <DropdownMenu 
            itemId={item.id} 
            itemType={media_type || 'unknown'}  // تمرير media_type أو قيمة افتراضية
            itemName={name}
