  ...
  useEffect(() => {
    const fetchMedia = async () => {
      try {
        ...
        // Fetch video data
        const videoData = await fetchFromTMDB(`/tv/${showId}/videos`);
        setMostPopularVideos(videoData?.results || []);

        // Fetch images data
        const imageData = await fetchFromTMDB(`/tv/${showId}/images`);
    ...
    if (showId) {
      fetchMedia(); // Fetch media if showId is available
    }
  }, [showId]);
  ...
  return (
    <div className="bg-white dark:bg-gray-900 text-black dark:text-white min-h-screen">
      <div className="container mx-auto px-4 py-8">
        ...
        {/* Section Navigation */}
        <div className="flex flex-wrap sm:flex-nowrap space-y-4 sm:space-y-0 sm:space-x-4 mb-6">
          <button
