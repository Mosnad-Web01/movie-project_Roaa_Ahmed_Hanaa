...
const YourRatingPage = () => {
  ...
  useEffect(() => {
    const fetchRatedItems = async () => {
        ...
        if (key.startsWith('movie-') || key.startsWith('tv-')) {
          ...
          if (data.rating) {
            try {
              ...
              allRatedItems.push({ ...itemData, type: key.split('-')[0] });
            } catch (error) {
      ...
      setRatedItems(allRatedItems);
    };

    fetchRatedItems();
  }, []);

  const filteredRatedItems = filter === 'all' ? ratedItems : ratedItems.filter(item => item.type === filter);
